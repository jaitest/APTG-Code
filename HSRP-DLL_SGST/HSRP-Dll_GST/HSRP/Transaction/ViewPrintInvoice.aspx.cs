        ...
        protected void buttonSave_Click(object sender, EventArgs e)
        {
          //  printDelevryChalan("PrintClose");
            ...
            try
            {
                ...
                HttpContext context = HttpContext.Current;
                string filename = "HSRP-CusInvoice" + System.DateTime.Now.Day.ToString() + System.DateTime.Now.Month.ToString() + System.DateTime.Now.Year.ToString() + System.DateTime.Now.Hour.ToString() + System.DateTime.Now.Minute.ToString() + ".pdf";
                ...
                SQlQuery = "select vehicletype+' (Damage Front)' as DescriptionOfGoods,count(*) as qty,round((sum(GSTBasicAmount)/count(*)),3) amt,round(sum(GSTBasicAmount),2)bamt,round(sum(UTGSTAmount),2)Vatamt,round(sum(CGSTAmount),2) excise,round(sum(cessamt),2)cess,round(sum(shecessamt),2)shecess  from hsrprecords  where hsrp_stateid='" + HSRPStateID + "' and ordertype in ('DF') and HsrpRecordID in(" + HSRPRecordID + ") group by vehicletype union   select vehicletype+' (Damage Rear)',count(*) as qty,round((sum(GSTBasicAmount)/count(*)),2) amt,round(sum(GSTBasicAmount),2)bamt,round(sum(UTGSTAmount),2)Vatamt,round(sum(CGSTAmount),2) excise,round(sum(cessamt),2)cess,round(sum(shecessamt),2)shecess   from hsrprecords  where hsrp_stateid='" + HSRPStateID + "' and HsrpRecordID in(" + HSRPRecordID + ") and  ordertype in ('DR') group by vehicletype union select vehicletype+' (Both Plates)',(count(*)*2)/2 qty,round((sum(GSTBasicAmount)/count(*)),2) amt,round(sum(GSTBasicAmount),2)bamt,round(sum(UTGSTAmount),2)Vatamt,round(sum(CGSTAmount),2) excise,round(sum(cessamt),2)cess,round(sum(shecessamt),2)shecess   from hsrprecords  where hsrp_stateid='2' and HsrpRecordID in(" + HSRPRecordID + ") and ordertype in ('NB','OB','DB') group by vehicletype";
                dt = Utils.GetDataTable(SQlQuery, CnnString);

                string SQlQuery1 = " select  a.OwnerName, a.Address1, Invoiceno,ChallanDate from hsrprecords a where hsrp_stateid='" + HSRPStateID + "' and HsrpRecordID in('" + HSRPRecordID + "')";
                DataTable dtname_Address = Utils.GetDataTable(SQlQuery1, CnnString);
                string strInvDate =Convert.ToDateTime(dtname_Address.Rows[0]["ChallanDate"]).ToString("dd/MM/yyyy");
                BAL obj = new BAL();
                ...
                if (true)
                {
                    ...
                    string straddress = GetAddress.Rows[0]["Address1"].ToString();
                    string strcity = GetAddress.Rows[0]["city"].ToString();
                    string strpincode = GetAddress.Rows[0]["pincode"].ToString();
                    ...
                    GenerateCellforhrading(table, 10, 1, 1, 1, 1, 1, 10, "Tax Invoice", 0, 0);
                    ...
                    decimal sgstpre = decimal.Round(Convert.ToDecimal(GetAddress.Rows[0]["UTGSTPer"].ToString()));
