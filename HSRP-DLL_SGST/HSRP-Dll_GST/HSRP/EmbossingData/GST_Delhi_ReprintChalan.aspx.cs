...
{
    public partial class GST_Delhi_ReprintChalan : System.Web.UI.Page
    {
        ...
        private void GetData()
        {
            ...
            string SQLString12 = " select row_number() over(order by hsrprecordid) as SN,ChallanNo, CashReceiptNo as 'Receipt Voucher No.',OwnerName as 'Customer Name',Vehicleregno as 'Vehicle No.',(select [NAME OF THE DEALER] from delhi_dealermaster where SNO=a.dealerid) as Dealer,(select [RtolocationName] from Rtolocation where rtolocationid=a.rtolocationid) as RtoLocationName,gstbasicamount as 'Taxable Value',cgstamount as 'CGST',UTgstamount as 'UTGST' ,RoundOff_NetAmount as 'Total Amount' from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and  [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'   union select   '','zz','','','','','Total' ,(  select sum(gstbasicamount ) from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' ) ,( select sum(cgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  ,( select sum(UTgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' )  ,(  select sum(RoundOff_NetAmount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  order by 2";
        ...
        private void GetRecords()
        {
            ...
            if (!string.IsNullOrEmpty(txtInvoiceNo.Text))
            {
                string SQLString = "select row_number() over(order by VehicleClass,VehicleType,VehicleRegNo) as SN,ChallanNo,challandate,convert(varchar,HSRPRecord_CreationDate,103) as HSRPRecord_CreationDate,VehicleType,convert(varchar,OrderEmbossingDate,103) as OrderEmbossingDate,hsrp_stateid,rtolocationid" +
                                    ",hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,OrderStatus  , CashReceiptNo as 'Receipt Voucher No.',OwnerName as 'Customer Name',Vehicleregno as 'Vehicle No.',(select [NAME OF THE DEALER] from delhi_dealermaster where sno=a.dealerid) as Dealer,gstbasicamount as 'Taxable Value',cgstamount as 'CGST',UTgstamount as 'UTGST' ,RoundOff_NetAmount as 'Total Amount'from hsrprecords a  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + txtInvoiceNo.Text + "' order by VehicleClass,VehicleType,VehicleRegNo";
            ...
            else if (!dropdownDuplicateFIle.SelectedItem.Text.Equals("--Select Challan No--"))
            {
                ...
                string SQLString = "select row_number() over(order by VehicleClass,VehicleType,VehicleRegNo) AS SN,ChallanNo,challandate,convert(varchar,HSRPRecord_CreationDate,103) as HSRPRecord_CreationDate,VehicleType,convert(varchar,OrderEmbossingDate,103) as OrderEmbossingDate,hsrp_stateid,rtolocationid" +
                                    ",hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,OrderStatus ,CashReceiptNo as 'Receipt Voucher No.',OwnerName as 'Customer Name',Vehicleregno as 'Vehicle No.', (select [NAME OF THE DEALER] from delhi_dealermaster where sno=a.dealerid) as Dealer,gstbasicamount as 'Taxable Value',cgstamount as 'CGST',UTgstamount as 'UTGST' ,RoundOff_NetAmount as 'Total Amount' from hsrprecords a  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  order by VehicleClass,VehicleType,VehicleRegNo";
                dtInvoiceData = Utils.GetDataTable(SQLString, CnnString);
        ...
        //                            "from hsrprecords where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + txtInvoiceNo.Text + "' and  convert(date,OrderEmbossingdate) <= convert(date,'2017-06-30') order by VehicleClass,VehicleType,VehicleRegNo";
        ...
        //        string SQLString = "select row_number() over(order by VehicleClass,VehicleType,VehicleRegNo) AS SN,ChallanNo,challandate,convert(varchar,HSRPRecord_CreationDate,103) as HSRPRecord_CreationDate,VehicleType,convert(varchar,OrderEmbossingDate,103) as OrderEmbossingDate,hsrp_stateid,rtolocationid" +
        //                            ",hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,OrderStatus " +
        //                            "from hsrprecords where hsrp_stateid='"+DropDownListStateName.SelectedValue.ToString()+"' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'    and  convert(date,OrderEmbossingdate) <= convert(date,'2017-06-30') order by VehicleClass,VehicleType,VehicleRegNo";
        //        dtInvoiceData = Utils.GetDataTable(SQLString, CnnString);
        //        strInvoiceNo = dropdownDuplicateFIle.SelectedItem.Text;
        //    }
        //    else
        ...
        protected void btnGO_Click(object sender, EventArgs e)
        {
            ...
            {                
                    ...
                    SQLString = "select  Distinct ChallanNo   from hsrprecords  where hsrp_StateID='" + DropDownListStateName.SelectedValue +
                        ...
                        strFrmDateString + "' and  '" + strToDateString + "'    and  convert(date,OrderEmbossingdate) >= convert(date,'2017-06-30') ";
             ...
             //   Button2.Visible = true;
               ...
               PdfPTable table3 = new PdfPTable(2);
                ...
                if (true)
                {
                    ...
                    //GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, "Date  -" + System.DateTime.Now.ToString("dd/MM/yyyy"), 0, 0);strInvDate
                    GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, "Date  -" + strInvDate, 0, 0);
                    GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, " ", 0, 0);
                    GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, " ", 0, 0);
                    ...
                    GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, "Transporter Name:"+ txtInvoiceNo.ToString().Trim(), 0, 0);
                    GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, "Affixation Center: " + strCompName, 0, 0);
                    GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, "Lorry  No.:", 0, 0);
                    ...
                    GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, " ", 0, 0);
                    GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, "State Code:  " + "7", 0, 0);
                    GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, " ", 0, 0);
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            Document document1 = new Document(new iTextSharp.text.Rectangle(188f, 124f), -30, -30, 9, 0);
            ...
            GenerateCell(table, 9, 0, 0, 0, 0, 1, 0, "(Annexure-" + dropdownDuplicateFIle.SelectedValue.ToString() + ")" + " Commodity - " + GetAddress.Rows[0]["CompanyName"].ToString() + ", (HSN Code - 8310)" + "\r\n", 0, 0);
            ...
            GenerateCell(table, 1, 0, 1, 1, 0, 0, 1, "LocationName", 0, 0);
            ...
            foreach (DataRow di in dtInvoiceData.Rows)
            {
                ...
                string LocationName = di["RtoLocationName"].ToString();
                string Taxablevalue = di["Taxable Value"].ToString();
                string strCGST = di["CGST"].ToString();
                string strSGST = di["UTGST"].ToString();
                ...
                GenerateCell(table, 1, 0, 1, 1, 1, 0, 1, LocationName, 0, 0);
