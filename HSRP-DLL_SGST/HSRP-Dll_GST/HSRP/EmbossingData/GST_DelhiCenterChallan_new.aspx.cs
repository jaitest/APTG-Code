...
{
    public partial class GST_DelhiCenterChallan_new : System.Web.UI.Page
    {
        ...
        private void Filldropdowndealer()
        {
            ...
            if (UserType == "0")
            {
               // SQLString = "select [NAME OF THE DEALER] from delhi_dealermaster  where ActiveStatus='Y' order by [name of the dealer]";
                ...
                string SQLString1 = "select distinct h.rtolocationid,a.[RtoLocationname]  from RtoLocation a,hsrprecords as h where a.rtolocationid=h.rtolocationid and H.hsrprecord_creationdate between '" + strFrmDateString + "' and '" + strToDateString + "' and  h.HSRP_StateID=2 ";
                Utils.PopulateDropDownList(ddlrto, SQLString1.ToString(), CnnString, "All");
        ...
        private DataTable GetRecords(string strRecordId)
        {
            ...
            string SQLString12 = "select  CashReceiptNo as 'Receipt Voucher No.',OwnerName as 'Customer Name',Vehicleregno as 'Vehicle No.', (select [RTolocationname] from rtolocation where rtolocationid=a.rtolocationid) as 'RTO Location Name',gstbasicamount as 'Taxable Value',cgstamount as 'CGST',utgstamount as 'SGST' ,RoundOff_NetAmount as 'Total Amount' from hsrprecords a  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and hsrprecordid=" + strRecordId + "  ";
        ...
        private void ShowGrid( string strFromdate,string strToDate)
        {
            ...
            string rtoid = ddlrto.SelectedValue;
            string Dname = string.Empty;
            string Did = string.Empty;
            string DealerId = string.Empty;
            //try
            //{
                //string SQLString1 = "select distinct Rtolocationid  from rtolocation a,hsrprecords as h where a.Rtolocationid=h.Rtolocationid and H.hsrprecord_creationdate between '" + strFromdate + "' and '" + strToDate + "' and  h.HSRP_StateID=2 and a.Rtolocationid='" + rtoid + "' ";
            ...
            try
            {
                if (rtoid == "All")
                {
                    ...
                    "from hsrprecords  as h,[rtolocation] as e where h.rtolocationid=E.rtolocationid AND h.hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and Invoice_Flag='N'  and e.NAVEMBID='" + ddlemb.SelectedValue + "'  " +
                     " and orderstatus ='Embossing Done' and ChallanNo is null and Hsrprecord_creationdate between '" + strFromdate + "' and '" + strToDate + "' and Addrecordby!='Dealer' and (hsrp_rear_lasercode is not null or hsrp_Front_lasercode is not null) " +
                ...
                else
                {
                    SQLString = "select e.[EmbCenterName], Row_Number() over(order by orderstatus) as SNo, hsrprecordid,HSRPRecord_AuthorizationNo,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,OrderStatus " +
                   "from hsrprecords  as h,[rtolocation] as e where h.rtolocationid=E.rtolocationid AND h.hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and Invoice_Flag='N'  and e.NAVEMBID='" + ddlemb.SelectedValue + "'  " +
                    " and orderstatus ='Embossing Done' and ChallanNo is null and Hsrprecord_creationdate between '" + strFromdate + "' and '" + strToDate + "' and Addrecordby!='Dealer' and h.rtolocationid='" + rtoid + "'  and (hsrp_rear_lasercode is not null or hsrp_Front_lasercode is not null)  and  convert(date,OrderEmbossingdate) >= convert(date,'2017-06-30')" +
                   "order by VehicleClass,VehicleType,VehicleRegNo";
                }
        ...
        protected void btnChalan_Click(object sender, EventArgs e)
        {
           // btnChalan.Visible = false;
            ...
            try
            {
                ...
                RtoName = ddlrto.SelectedItem.ToString();
                HttpContext context = HttpContext.Current;
                //string dealername2 = ddlBothDealerHHT.SelectedItem.ToString();
                string filename = "HSRP-DC_" + RtoName.ToString().Trim() + System.DateTime.Now.Day.ToString() + System.DateTime.Now.Month.ToString() + System.DateTime.Now.Year.ToString() + System.DateTime.Now.Hour.ToString() + System.DateTime.Now.Minute.ToString() + ".pdf";
                ...
                for (int i = 0; i < GridView1.Rows.Count; i++)
                {
                    ...
                    if (chk.Checked == true)
                    {
                        ...
                        sb.Append("update hsrprecords set ChallanNo='" + strInvoiceNo + "', ChallanCreatedBy='" + InvoiceUser + "', challandate=getdate(),Invoice_Flag='Y' where hsrp_StateID=2 and hsrprecordid ='" + strRecordId + "' and  isnull( ChallanNo ,'')=''  and  isnull( challandate ,'')='' and  Invoice_Flag ='N'  ; ");
                       
                        //changes for Customer Invoice 
                        string strGetcustomerInvoiceNo = " select (prefixtext+right('000000'+ convert(varchar,lastno+1),6))   from prefix  where hsrp_stateid= 2  and  prefixfor = 'Customer Invoice'  and  RTOLocationID ='" + ddlrto.SelectedValue.ToString() + "' ";
                        ...
                        if (cus > 0)
                        {
                            Utils.ExecNonQuery("update prefix set LastNo =  (select right('000000'+ convert(varchar,lastno+1),6)  from prefix  where hsrp_stateid= '" + DropDownListStateName.SelectedValue + "'  and  prefixfor = 'Customer Invoice' and  RTOLocationID ='" + ddlrto.SelectedValue.ToString() + "' )  where hsrp_stateid= '" + HSRPStateID + "'  and  prefixfor = 'Customer Invoice' and  RTOLocationID ='" + ddlrto.SelectedValue.ToString() + "' ", CnnString);
                        }
                ...
                string sqlstring = string.Empty;
                sqlstring = "insert into InvoiceMaster(InvoiceNo,InvoiceDate,Amount,BuyerName,clientName,hsrp_stateid,dispatchedLocation) values('" + strInvoiceNo + "', Convert(date,('" + currentdate + "'),103),'" + totalamount + "','" + RtoName + "','" + GetAddress.Rows[0]["Address1"].ToString() + "','" + DropDownListStateName.SelectedValue + "','" + ddlrto.SelectedItem.Text + "')";
                Utils.ExecNonQuery(sqlstring, CnnString);



                string strSelectAddress = "SELECT RTOLocationAddress FROM [RTOLocation] WHERE RTOLocationId='" + ddlrto.SelectedValue + "'";
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            GenerateCell(table, 1, 0, 1, 1, 0, 0, 1, "RTO Location Name", 0, 0);
            ...
            foreach (GridViewRow di in GridView1.Rows)
            {
                ...
                if (chkBx != null && chkBx.Checked)
                {
                    ...
                    if (dtData.Rows.Count > 0)
                    {
                        ...
                        string dealername = dtData.Rows[0]["RTO Location Name"].ToString();
        ...
        protected void btnGO1_Click(object sender, EventArgs e)
        {
           ...
           // string ddVehicleType = DropDownList1.SelectedItem.ToString().ToUpper();
            ...
            string DealerName = ddlrto.SelectedValue;
            string Dname = string.Empty;
            string Did = string.Empty;
            string DealerId = string.Empty;
            try
            {
                ...
                ddlrto.Visible = true;
                string SQLString1 = "select distinct h.Rtolocationid,[Rtolocationname]  from Rtolocation a,hsrprecords as h where a.Rtolocationid=h.Rtolocationid and H.Invoice_Flag='N' and H.hsrprecord_creationdate between '" + strFrmDateString + "' and '" + strToDateString + "' and h.[NAVEMBID]='" + ddlemb.SelectedValue+ "' and  h.HSRP_StateID=2 order by [Rtolocationname] ";
              //  DealerId = Utils.getDataSingleValue(SQLString1, CnnString, "dealerid");
                Utils.PopulateDropDownList(ddlrto, SQLString1.ToString(), CnnString, "All");
            }
            catch (Exception ex)
