...
namespace HSRP.Transaction
{
    public partial class Holiday : System.Web.UI.Page
    {
        ...
        private void InitialSetting()
        {
            ...
            OrderDate.MinDate = DateTime.Parse(MaxDate);
        ...
        protected void buttonSave_Click(object sender, EventArgs e)
        {
           ...
           // string ToDate = From1 + " 23:59:59";
            ...
            try
            {
                string sqlquery = "Select * from HolidayDateTime where hsrp_stateid='" + stateid + "' and convert(date,blockDate)=convert(date,'" + AuthorizationDate + "')";
                DataTable dtDuplicate= Utils.GetDataTable(sqlquery, CnnString);
                if (dtDuplicate.Rows.Count > 0)
                {
                    lblSucMess.Text = "";
                    lblErrMess.Text = "Holiday already exist corresponding your choose date";
                    return;
                }
                else 
                {
                    ...
                    if (count > 0)
                    {
                        if (count == 2627)
                        {
                            lblSucMess.Text = "Holiday Date Already Exist.";
                        }
                        else
                        {
        ...
        protected void btnReset_Click(object sender, EventArgs e)
        {
            ...
            ddlEmbossingCenter.ClearSelection();
            txtdesc.Text = "";
            lblErrMess.Text = "";
            lblSucMess.Text = "";
            InitialSetting();
