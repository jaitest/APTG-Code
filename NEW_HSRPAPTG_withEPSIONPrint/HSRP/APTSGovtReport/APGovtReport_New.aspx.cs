...
{
    public partial class APGovtReport : System.Web.UI.Page
    {
        ...
        DataTable dt12345 = new DataTable();
        ...
        //}
        // AP GET AUTH NO from Govt
        private void APGenerateExcelReport()
        {
            ...
            try
            {
                ...
                if (AuthData.Length > 1)
                {
                    ...
                    dt12345.Columns.Add("Authno", typeof(string));
                    ...
                    using (XmlTextReader reader = new XmlTextReader(stringReader))
                    {
                        while (reader.Read())
                        {
                            ...
                            if (reader.Name.ToString() == "AuthNo")
                            {
                                ...
                                else
                                {
                                    ...
                                    string t;
                                    var qry = from DataRow row12 in dtauth.Rows
                                              let arr = row12.ItemArray
                        ...
                        if (dt12345.Rows.Count > 0)
                        {
                            Int64 sno = 0;
                            foreach (DataRow dtrows in dt12345.Rows) // Loop over the rows.
                            {                               
        ...
        {
            APGenerateExcelReport();
        }

        private void APGenerateExcelReportap(string strRepotType)
        {
            string RTOName = string.Empty;
            ...
            try
            {
               ...
               // SetFolder(RTOName, "AP", "Royalty and Tax-");
                ...
                else
                {
                    if (strRepotType == "OnlineRecords")
                    {
                        SQLString = "select r.Rtolocationname as RTO, h.HSRPRecord_AuthorizationNo as AuthNo ,(convert(varchar(15),h.HSRPRecord_AuthorizationDate,103)) as AuthDate,h.Engineno,h.Chassisno, h.VehicleType,h.vehicleclass, h.vehicleregno as RegNo,(convert(varchar(15),h.orderdate,103)) as OrderDate from HSRPRecords h,RTOLocation r where h.RTOLocationID=r.rtolocationid and h.HSRP_StateID=9 and h.addrecordby!='WebApp' and isnull(h.vehicleregno,'')='' and h.HSRPRecord_AuthorizationNo not like 'AP%'  Order by AuthNo,OrderDate";
                    }
                    else if (strRepotType == "OfflineRecords")
                    {
                        SQLString = "select r.Rtolocationname as RTO, h.HSRPRecord_AuthorizationNo as AuthNo ,(convert(varchar(15),h.HSRPRecord_AuthorizationDate,103)) as AuthDate,h.Engineno,h.Chassisno, h.VehicleType,h.vehicleclass, h.vehicleregno as RegNo,(convert(varchar(15),h.orderdate,103)) as OrderDate from HSRPRecords h,RTOLocation r where h.RTOLocationID=r.rtolocationid and h.HSRP_StateID=9 and h.addrecordby='WebApp' and isnull(h.vehicleregno,'')='' and h.HSRPRecord_AuthorizationNo like 'AP%' Order by AuthNo,OrderDate";
                    }
                }
        ...
        {
            APGenerateExcelReportap("OnlineRecords");
        }

        protected void btnOfflineAPReport_Click(object sender, EventArgs e)
        {
            APGenerateExcelReportap("OfflineRecords");
        }
