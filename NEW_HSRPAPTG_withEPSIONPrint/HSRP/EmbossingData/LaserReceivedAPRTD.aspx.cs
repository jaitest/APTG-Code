...
{
    public partial class LaserReceivedAPRTD : System.Web.UI.Page
    {
            ...
            for (int i = 0; i < GridView1.Rows.Count; i++)
            {
                ...
                if (chk.Checked == true)
                {
                        ...
                        else
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    SqlQuery = "insert into TSSMSDetail (HSRPRecordID,RtoLocationID,VehicleRegNo,MobileNo,SentResponseCode,SMSTEXT) values('" + strRecordId + "','" + RTOLocationID + "','" + lblVehicleRegNo.Text + "','" + paymentmobile + "','" + result + "','" + SMSText + "')";
                                    Utils.ExecNonQuery(SqlQuery, CnnString);
                                }
                            }
                            else
                    ...
                    else if (HSRPStateID == "9")
                    {
                        ...
                        string affixationCenterCode = Utils.getScalarValue("Select affixationCenterCode from hsrpdata where authorizationRefNo='" + strAuthNo + "'", CnnString);
                        string Json = "{\"affixationCenterCode\":\"" + affixationCenterCode + "\",\"hsrpAvailabilityDate\":\"" + System.DateTime.Now.ToString("dd/MM/yyyy") + "\",\"authRefNo\":\"" + strAuthNo + "\"}";
                        List<Response> Responseap = HSRPAvailability(Json, Utils.Login());
                        string status = Responseap[0].status.ToString();
                        string message = Responseap[0].message.ToString();
                        Utils.ExecNonQuery("update hsrprecords set apwebservicerespplatatofficeMsg='" + message + "',apwebservicerespplatatoffice='" + status + "',apwebservicerespplatatofficedatetime=Getdate() where HSRPRecordID='" + strRecordId + "' and vehicleregno='" + lblVehicleRegNo.Text + "'", CnnString);
                        if (PhoneNumber.Length > 0)
                        {
                            ...
                            if (HSRPRecordID.Rows.Count == 0)
                            {
                                ...
                                SqlQuery = "insert into APSMSDetail (HSRPRecordID,RtoLocationID,VehicleRegNo,MobileNo,SentResponseCode,SMSTEXT) values('" + strRecordId + "','" + RTOLocationID + "','" + lblVehicleRegNo.Text + "','" + PhoneNumber + "','" + result + "','" + SMSText + "')";
                                Utils.ExecNonQuery(SqlQuery, CnnString);
                            }
                        }
                        else
        ...
        public List<Response> HSRPAvailability(string locationJSON, string token)
        {
            string URL = System.Configuration.ConfigurationManager.AppSettings["APRTDHSRPAvailabilityURL"].ToString();
