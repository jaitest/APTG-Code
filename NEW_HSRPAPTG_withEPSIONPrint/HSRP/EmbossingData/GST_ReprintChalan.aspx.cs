...
{
    public partial class GST_ReprintChalan : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //if (invoivemaster().ToString() == string.Empty)
                //{
                //    lblpckweight.Visible = true;
                //    lblwaybillno.Visible = true;
                //    txtpckweight.Visible = true;
                //    txtwaybillno.Visible = true;
                //}
                //else
                //{
                //    lblpckweight.Visible = false;
                //    lblwaybillno.Visible = false;
                //    txtpckweight.Visible = false;
                //    txtwaybillno.Visible = false;
                //}
            }
        }
        ...
        private void GetRecords()
        {
            ...
            if (!string.IsNullOrEmpty(txtInvoiceNo.Text))
            {
                string SQLString = "select row_number() over(order by VehicleClass,VehicleType,VehicleRegNo) as SN,ChallanNo,challandate,convert(varchar,HSRPRecord_CreationDate,103) as HSRPRecord_CreationDate,VehicleType,convert(varchar,OrderEmbossingDate,103) as OrderEmbossingDate,hsrp_stateid,rtolocationid" +
                                    ...
                                    "from hsrprecords where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + txtInvoiceNo.Text + "' and convert(date,OrderEmbossingDate,103)>='2017-07-01' order by VehicleClass,VehicleType,VehicleRegNo";
            ...
            else if (!dropdownDuplicateFIle.SelectedItem.Text.Equals("--Select Challan No--"))
            {
                string SQLString = "select row_number() over(order by VehicleClass,VehicleType,VehicleRegNo) AS SN,ChallanNo,challandate,convert(varchar,HSRPRecord_CreationDate,103) as HSRPRecord_CreationDate,VehicleType,convert(varchar,OrderEmbossingDate,103) as OrderEmbossingDate,hsrp_stateid,rtolocationid" +
                                    ",hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,OrderStatus " +
                                    "from hsrprecords where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' and convert(date,OrderEmbossingDate,103)>='2017-07-01' order by VehicleClass,VehicleType,VehicleRegNo";
                dtInvoiceData = Utils.GetDataTable(SQLString, CnnString);
        ...
        private void InitialSetting()
        {
            ...
            //string strSelInvoiceData = "Select pckweight,WaybillNo from invoicemaster where InvoiceNo='" + strInvoiceNo + "'";
            //DataTable dtInvoicemaster = Utils.GetDataTable(strSelInvoiceData, CnnString);
        ...
        {
            BindGrid();
            if (invoivemaster(dropdownDuplicateFIle.SelectedItem.ToString()).ToString() == string.Empty)
            {
                lblpckweight.Visible = true;
                lblwaybillno.Visible = true;
                txtpckweight.Visible = true;
        ...
        public string invoivemaster(string invoiceno)
        {
            string strSelInvoiceData = "Select pckweight,WaybillNo from invoicemaster where InvoiceNo='" + invoiceno + "'";
            DataTable dtInvoicemaster = Utils.GetDataTable(strSelInvoiceData, CnnString);
            if (dtInvoicemaster.Rows.Count > 0)
            {
                return dtInvoicemaster.Rows[0]["pckweight"].ToString();
            }
            else
            {
                return "";
        ...
        protected void btnChalan_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lblErrMsg.Text = "Please enter valid Invoice Number.";
                    return;
                }
                if (invoivemaster(dropdownDuplicateFIle.SelectedItem.ToString()).ToString() == string.Empty)
                {
                    if (string.IsNullOrEmpty(txtpckweight.Text))
                    {
                        lblErrMsg.Text = "Please enter Packing Weight.";
                        return;
                    }
                    if (string.IsNullOrEmpty(txtwaybillno.Text))
                    {
                        lblErrMsg.Text = "Please enter Way Bill Number.";
                        return;
                    }
                }
                ...
                string strSelInvoiceData = "Select ID,transporter,lorryno,pckweight,WaybillNo from invoicemaster where InvoiceNo='" + strInvoiceNo + "'";
                DataTable dtInvoice = Utils.GetDataTable(strSelInvoiceData, CnnString);
                string strTransporter = string.Empty; ;
                string strLorryNo = string.Empty;
                string pckweight = string.Empty;
                string waybillno = string.Empty;
                if (dtInvoice.Rows.Count > 0)
                {
                    strTransporter = dtInvoice.Rows[0]["Transporter"].ToString();
                    strLorryNo = dtInvoice.Rows[0]["LorryNo"].ToString();
                    if (txtpckweight.Text.ToString() != string.Empty)
                    {
                        pckweight = txtpckweight.Text;
                        Utils.ExecNonQuery("update invoicemaster set pckweight='" + txtpckweight.Text + "' where id='" + dtInvoice.Rows[0]["ID"].ToString() + "'", CnnString);
                    }
                    else
                    {
                        pckweight = dtInvoice.Rows[0]["pckweight"].ToString();
                    }
                    if (txtwaybillno.Text.ToString() != string.Empty)
                    {
                        waybillno = txtwaybillno.Text;
                        Utils.ExecNonQuery("update invoicemaster set WaybillNo='" + txtwaybillno.Text + "' where id='" + dtInvoice.Rows[0]["ID"].ToString() + "'", CnnString);
                    }
                    else
                    {
                        waybillno = dtInvoice.Rows[0]["WaybillNo"].ToString();
                    }
                }
                else
                {
                ...
                if (true)
                {
                   ...
                   // string strStateId = DropDownListStateName.SelectedValue;
                    ...
                    if (!strEmbStationName.Equals("Chintal"))
                    {
                        ...
                        GenerateCell(table, 5, 0, 1, 0, 0, 0, 1, "Date" + " : " + strInvDate, 0, 0);
                    ...
                    if (!strEmbStationName.Equals("Chintal"))
                    {
                        ...
                        GenerateCell(table, 5, 0, 1, 1, 0, 0, 1, "Transporter Name:" + txtpckweight.Text.ToString(), 0, 0);
                        GenerateCell(table, 5, 1, 1, 0, 0, 0, 1, "Affixation Center: " + strCompName, 0, 0);
                        GenerateCell(table, 5, 0, 1, 0, 0, 0, 1, "Lorry No.:" + txtwaybillno.Text.ToString(), 0, 0);
        ...
        private void GetData()
        {
            try
            {
                ...
                { 
                    dtInvoiceData = new DataTable();
                   // string SQLString12 = " select row_number() over(order by hsrprecordid) as SN,ChallanNo, CashReceiptNo as 'Receipt Voucher No.',OwnerName as 'Customer Name',Vehicleregno as 'Vehicle No.',  (select dealername+'-'+City from dealermaster where dealerid=a.dealerid) as Dealer,gstbasicamount as 'Taxable Value',cgstamount as 'CGST',sgstamount as 'SGST' ,RoundOff_NetAmount as 'Total Amount' from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and  [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'   union select   '','zz','','','','Total' ,(  select sum(gstbasicamount ) from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' ) ,( select sum(cgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  ,( select sum(sgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' )  ,(  select sum(RoundOff_NetAmount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  order by 2";
                    string SQLString12 = " select row_number() over(order by hsrprecordid) as SN,ChallanNo, CashReceiptNo as 'Receipt Voucher No.',OwnerName as 'Customer Name',Vehicleregno as 'Vehicle No.',  (select dealername+'-'+City from dealermaster where dealerid=a.dealerid) as Dealer,gstbasicamount+(-Roundoff_value) as 'Taxable Value',cgstamount as 'CGST',sgstamount as 'SGST' ,RoundOff_NetAmount as 'Total Amount' from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and  [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'   union select   '','zz','','','','Total' ,(select sum(gstbasicamount+(-Roundoff_value) ) from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' ) ,( select sum(cgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  ,( select sum(sgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' )  ,(  select sum(RoundOff_NetAmount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  order by 2";
                ...
                {
                    dtInvoiceData = new DataTable();
                   // string SQLString12 = " select row_number() over(order by hsrprecordid) as SN,ChallanNo, CashReceiptNo as 'Receipt Voucher No.',OwnerName as 'Customer Name',Vehicleregno as 'Vehicle No.','' as dealer,gstbasicamount as 'Taxable Value',cgstamount as 'CGST',sgstamount as 'SGST' ,RoundOff_NetAmount as 'Total Amount' from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and  [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'   union select   '','zz','','','','Total' ,(  select sum(gstbasicamount ) from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' ) ,( select sum(cgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  ,( select sum(sgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' )  ,(  select sum(RoundOff_NetAmount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  order by 2";
                    string SQLString12 = " select row_number() over(order by hsrprecordid) as SN,ChallanNo, CashReceiptNo as 'Receipt Voucher No.',OwnerName as 'Customer Name',Vehicleregno as 'Vehicle No.','' as dealer,gstbasicamount+(-Roundoff_value) as 'Taxable Value',cgstamount as 'CGST',sgstamount as 'SGST' ,RoundOff_NetAmount as 'Total Amount' from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and  [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'   union select   '','zz','','','','Total' ,(  select sum(gstbasicamount+(-Roundoff_value) ) from hsrprecords a where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' ) ,( select sum(cgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  ,( select sum(sgstamount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "' )  ,(  select sum(RoundOff_NetAmount) from hsrprecords  where hsrp_stateid='" + DropDownListStateName.SelectedValue.ToString() + "' and [ChallanNo]='" + dropdownDuplicateFIle.SelectedItem.Text + "'  )  order by 2";
                    dtInvoiceData = Utils.GetDataTable(SQLString12, CnnString);
                    if (dtInvoiceData.Rows.Count > 0)
                    {
                        btnChalan.Visible = true;
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            GetData();          
            DataTable GetAddress = Utils.GetDataTable("select * from HSRPState WHERE HSRP_StateID='" + HSRPStateID + "'", CnnString);
            ...
            GenerateCell(table, 9, 0, 0, 0, 0, 1, 0, "(Annexure-" + dropdownDuplicateFIle.SelectedValue.ToString() + ")" + "  Commodity - " + GetAddress.Rows[0]["CompanyName"].ToString() + ", (HSN Code - 8310)", 0, 0);
            ...
            {
                string sno;
                if (di["SN"].ToString().Trim() == "0")
                {
                    sno = "";
                }
                else
                {
                    sno = di["SN"].ToString().Trim();
                } 
                string Receipt = di["Receipt Voucher No."].ToString();
                string Customer = di["Customer Name"].ToString();
                string Vehicleno = di["Vehicle No."].ToString();
                string dealername = di["Dealer"].ToString();
                string Taxablevalue = di["Taxable Value"].ToString();
                string strCGST = di["CGST"].ToString();
                string strSGST = di["SGST"].ToString();
                string amount = di["Total Amount"].ToString();

                GenerateCell(table, 1, 1, 1, 1, 1, 0, 1, sno, 0, 0);
                GenerateCell(table, 1, 0, 1, 1, 1, 0, 1, Receipt, 0, 0);
