...
namespace HSRP.EmbossingData
{
    public partial class APChalan : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                for (int i = 0; i < GridView1.Rows.Count; i++)
                {
                    ...
                    if (chk.Checked == true)
                    {
                        ...
                        //string[] hsrprecordidQty = strHsrpRecordId.ToString().Trim().Split(',');
                        //int j = hsrprecordidQty.Length;
                        //string hsrprecordid = hsrprecordidQty[j - 1].ToString().Trim();
                        //dr["hsrprecordid"] = hsrprecordid;
                ...
                if (iChkCount > 0)
                {
                    ...
                    if (dt1.Rows.Count > 0)
                    {
                        ...
                        {
                            //if (Utils.ExecNonQuery(sbx.ToString(), CnnString) > 0)
                            //{
                            //    if (HSRPStateID == "9" || HSRPStateID == "11" || HSRPStateID == "4" || HSRPStateID == "3" || HSRPStateID == "6" || HSRPStateID == "1")
                            //    {
                            ...
                            if (true)
                            {
                                ...
                                GetAddress.Rows[0]["city"].ToString() + "-"+pincode + " E-Mail : info@linkutsav.com \r\n" + strPhone, 0, 0);
                               ...
                               // Changes By vimal
                                ...
                                //GenerateCell(table, 5, 1, 0, 0, 0, 0, 1, "", 0, 0);
                                ...
                                //GenerateCell(table, 4, 1, 0, 0, 0, 0, 1,"", 0, 0);
                                GenerateCell(table, 1, 0, 1, 0, 0, 2, 1, "", 0, 0);

                                //GenerateCell(table, 3, 1, 0, 0, 0, 0, 1, "SNAP LOCK 19(Qty.)", 0, 0);
                                //GenerateCell(table, 1, 0, 0, 0, 0, 0, 1, txtSnapLockQuantity19.Text.ToString(), 0, 0);
                                //GenerateCell(table, 12, 0, 1, 0, 0, 0, 1, "", 0, 0);

                                //GenerateCell(table, 3, 1, 0, 0, 0, 0, 1, "SNAP LOCK 25(Qty.)", 0, 0);
                                //GenerateCell(table, 1, 0, 0, 0, 0, 0, 1, txtSnapLockQuantity25.Text.ToString(), 0, 0);
                                //GenerateCell(table, 12, 0, 1, 0, 0, 0, 1, "", 0, 0);
                                ...
                                //GenerateCell(table3, 2, 0, 0, 0, 0, 2, 1, "1", 0, 0);
        ...
        {
            //string sqlquery = "Select ProductID,Productcode,productdimension from Product where hsrp_stateid='" + HSRPStateID + "' and Productcode='SNAP LOCK' and ActiveStatus='Y' order by 1";
            //DataTable dt = Utils.GetDataTable(sqlquery, CnnString);
            //if (dt.Rows.Count > 0)
            //{
            //    hdnSNAPLOCK19.Value = dt.Rows[0]["ProductID"].ToString();
            //    hdnSNAPLOCK25.Value = dt.Rows[1]["ProductID"].ToString();
            //}
        }
