...
{
    public partial class Affixation : System.Web.UI.Page
    {
        ...
        private void ShowGrid()
        {
            ...
            {
                string SQLString = "select hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,HSRPRecord_AuthorizationNo from hsrprecords where hsrp_stateid ='" + HSRPStateID + "' and PdfFileName='" + dropdownDuplicateFIle.SelectedItem.ToString() + "' and orderstatus ='Embossing Done' and isnull(OwnerName,'')!='' and addrecordby='Dealer' order by VehicleClass,VehicleType,VehicleRegNo";
            ...
            try
            {
                ...
                for (int i = 0; i < GridView1.Rows.Count; i++)
                {
                    ...
                    if (chk.Checked == true)
                    {
                        ...
                        sb.Append("update hsrprecords set orderstatus='closed',[OrderclosedDate]=getdate() , closeduserid='" + strUserID + "' where HSRPRecordID='" + id.Text + "' and vehicleregno='" + lblVehicleRegNo.Text + "'" + Environment.NewLine);
                    }
                }
                ...
                if (DropDownListStateName.SelectedValue.ToString() == "9")
                {
                    ...
                    for (int i = 0; i < GridView1.Rows.Count; i++)
                    {
                        ...
                        if (chk.Checked == true)
                        {
                            ...
                            objAPService.UpdateHSRPLaserCodes(lblAuthNo.Text, FLasercode.Text, RearLaserCode.Text, DateTime.Now.ToString("dd/MM/yyyy"));
                        }
                    }
                }
                if (DropDownListStateName.SelectedValue.ToString() == "11")
                {
                    ...
                    for (int i = 0; i < GridView1.Rows.Count; i++)
                    {
                        ...
                        if (chk.Checked == true)
                        {
                            ...
                            objTGService.UpdateHSRPLaserCodes(lblAuthNo.Text, FLasercode.Text, RearLaserCode.Text, DateTime.Now.ToString("dd/MM/yyyy"));
                        }
                    }
                }
                if (j > 0)
