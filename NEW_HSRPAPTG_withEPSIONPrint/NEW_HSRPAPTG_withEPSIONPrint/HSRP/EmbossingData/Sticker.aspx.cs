...
namespace HSRP.EmbossingData
{
    public partial class Sticker1 : System.Web.UI.Page
    {
        ...
        protected void btnGO_Click(object sender, EventArgs e)
        {
            ...
            SQLString = "select Distinct  pdffilename  from hsrprecords where hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and rtolocationID='" + dropDownListClient.SelectedValue + "' and  pdffilename is not null and erpassigndate between '" + AuthorizationDate + "' and  '" + ToDate + "'  group by  pdffilename, convert(varchar,pdfdownloaddate,111)";
        ...
        private void ShowGridDelhi()
        {
           ...
           string DealerTab=ddlBothDealerHHT.SelectedItem.ToString();
            if (dropdownDuplicateFIle.SelectedItem.Text != "--Select RTO Name--")
            {
                ...
                {
                    string DealerName = ddlBothDealerHHT.SelectedValue.ToString();
                    string Dname = string.Empty;
                    string Did = string.Empty;

                    DealerId = "select distinct dealerid,(select [NAME OF THE DEALER] from delhi_dealermaster a where a.SNO=hsrprecords.dealerid) as DealerName from hsrprecords where HSRP_StateID=2 and Addrecordby='Dealer'";
                    ...
                    for (int i = 0; dtdealerid.Rows.Count > i; i++)
                    {
                        ...
                        if (Dname == DealerName)
                        {
                            ...
                            string SQLString = "select hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,OrderStatus from hsrprecords where OrderStatus ='Embossing Done' and DealerId='" + Did + "' and PdfFileName='" + dropdownDuplicateFIle.SelectedItem.ToString() + "' and hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and rtolocationID='" + dropDownListClient.SelectedValue + "' and StickerMandatory='Y'";
