...
namespace HSRP.TG
{
    public partial class TGOnlineOrderBooked : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                for (int i = 0; i < GridView1.Rows.Count; i++)
                {
                    ...
                    if (chk.Checked == true)
                    {
                        ...
                        DataTable dtauthno = Utils.GetDataTable("Select HSRPRecord_AuthorizationNo from TGOnlinePaymentOrderBooked where HSRPRecordID='" + OrderBookedId + "' and isnull(ReSyncUserId,'')=''", CnnString);
                        if (dtauthno.Rows.Count > 0)
                        {
                            if (txtAuthNo.Text.ToString().ToUpper() == dtauthno.Rows[0]["HSRPRecord_AuthorizationNo"].ToString().ToUpper())
                            {
                                sbx.Append("update TGOnlinePaymentOrderBooked Set paymentgateway='" + ddlGateway.Text.ToString().Trim() + "',btninitiatedfrom='" + ddlGateway.Text.ToString().Trim() + "',onlinepaymenttrackingno='" + TextBox1.Text.ToString().Trim() + "', ReSyncUserId='" + strUserID + "',ReSyncDatetime=getdate(), ReSyncStatus='N' where HSRPRecordID='" + OrderBookedId + "'");
                            }
                        }
                        else
                        {
                ...
                if (iChkCount == 1)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        ddlGateway.ClearSelection();
                        TextBox1.Text = "";
                        //int c = Utils.ExecNonQuery("GetTGOnlinePaymentOrderBookingHSRP '" + OrderBookedId + "'", CnnString);
                        //if (c > 0)
                        //{
                        //    LblMessage.Text = "Record Booked successfully";
                        //}
        ...
        public string SaveOrderBooked(string id)
        {
            using (SqlConnection con = new SqlConnection(CnnString))
            {
                ...
                try
                {
                    SqlCommand cmd = new SqlCommand("[GetTGOnlinePaymentForOrderBooking]", con);
