...
namespace TenderReports
{
    public partial class Form1 : Form
    {
        ...
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ConnectionStringAPTG"].ConnectionString);
        string CnnString = ConfigurationManager.ConnectionStrings["ConnectionStringAPTG"].ConnectionString;
        ...
        public void button1_Click(object sender, EventArgs e)
        {
            ...
            rtoaddr = GetDataInDT("select RTOLocationAddress from rtolocation where RTOLocationID=258 ");
...
//string sqlquery = "select Address1 from AffixationCenters where Rto_Id='" + ddlaffixation.SelectedValue.ToString() + "'";
           // string  AffAddress = Utils.getDataSingleValue(sqlquery, ConnectionString, "Address1");
            ...
            GetAddress = GetDataInDT("select * from HSRPState WHERE HSRP_StateID=6");
            ...
            string getTinNo = Utils.getScalarValue("select TinNo from hsrpstate where HSRP_StateID=6",CnnString);

            DataTable dtvechilereg_no = new DataTable();
            dtvechilereg_no = Utils.GetDataTable("select VehicleRegNo from hsrprecords where hsrp_stateid = 6 and rtolocationid =258 and VehicleRegNo not in('UK07AL3087','UK07AM4675','UK07AH5593','123456') and HSRPRecord_CreationDate between '04/01/2013 00:00:00' and '03/31/2014 23:58:00' ", CnnString);
            for(int i=0; dtvechilereg_no.Rows.Count>i; i++)
            {
                string vegno = dtvechilereg_no.Rows[i]["VehicleRegNo"].ToString();

                string SQLString = "select hsrprecordID, HSRPRecord_CreationDate,HSRP_StateID,RTOLocationID,HSRPRecord_AuthorizationNo,HSRPRecord_AuthorizationDate,VehicleRegNo,OwnerName,MobileNo,VehicleClass,OrderType,StickerMandatory,IsVIP,NetAmount,VehicleType,CashReceiptNo from hsrprecords where  VehicleRegNo ='" + vegno + "'";
            ...
            if (dataSetFillHSRPDeliveryChallan.Rows.Count > 0)
            {
                ...
                string AuthDate =dataSetFillHSRPDeliveryChallan.Rows[0]["HSRPRecord_AuthorizationDate"].ToString();
               // string date = AuthDate.ToString("dd/MM/yyyy");

                PdfPCell cell255 = new PdfPCell(new Phrase(": " + AuthDate, new iTextSharp.text.Font(bfTimes, 8f, iTextSharp.text.Font.NORMAL, iTextSharp.text.BaseColor.BLACK)));
                cell255.Colspan = 1;
