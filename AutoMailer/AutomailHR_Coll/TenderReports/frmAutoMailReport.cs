        ...
        private void Export()
        {
            try
            {
                ...
                else
                {
                    ...
                    strtodate = System.DateTime.Now.AddDays(-1).ToString("MM-dd-yyyy");
                    //strtodate = System.DateTime.Now.ToString();
                }

                //string SQLStrings = "exec USP_TenderReport_HR_Schedule_F";
                string SQLStrings = "exec Affix_FaridabadRTA";
                ...
                if (dtsp.Rows.Count > 0)
                {
                    for (int Temp=0; Temp < dtsp.Rows.Count; Temp++) 
                    {
                       ...
                       // string strlocationid = string.Empty;
                        ...
                        GenerateCell(table, 3, 0, 0, 0, 0, 0, 0, "Report Date :" + strfromdate , 20, 0);
                        GenerateCell(table, 13, 0, 0, 0, 0, 0, 0, "", 20, 0);
                        ...
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "CashReceiptno", 40, 0);
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "CashReceiptDate", 40, 0);
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "AuthNo", 40, 0);
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "orderdate", 40, 0);
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "VehicleNo", 40, 0);
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "OwnerName", 40, 0);
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "Vehicle Type", 40, 0);
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "Vehicle Class", 20, 0);
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "AffixationDate", 40, 0);
                        GenerateCell(table, 1, 0, 1, 1, 1, 1, 0, "CreatedBy", 40, 0);
            ...
            #region Dynamic Rows
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            lblmsg.Text = "Not Get FaridabadRTA Collection Today";
                           // return;
                            document.Open();
                        ...
                        else 
                        {
                            ...
                            foreach (DataRow dtrows in dt.Rows) // Loop over the rows.
                            {
                                ...
                                GenerateCell(table, 1, 0, 1, 0, 1, 1, 1, dtrows["CashReceiptDate"].ToString(), 20, 0);
                                GenerateCell(table, 1, 0, 1, 0, 1, 1, 1, dtrows["AuthNo"].ToString(), 20, 0);
                                GenerateCell(table, 1, 0, 1, 0, 1, 1, 1, dtrows["orderdate"].ToString(), 20, 0);
                                GenerateCell(table, 1, 0, 1, 0, 1, 1, 1, dtrows["VehicleNo"].ToString(), 20, 0);
                                ...
                                GenerateCell(table, 1, 0, 1, 0, 1, 1, 1, dtrows["AffixationDate"].ToString(), 20, 0);
                                GenerateCell(table, 1, 0, 1, 0, 1, 1, 1, dtrows["CreatedBy"].ToString(), 20, 0);
                                GenerateCell(table, 1, 0, 1, 0, 1, 1, 1, dtrows["Amount"].ToString(), 20, 0);
            #endregion
                            }

                            string filename = "FaridabadRTA Collection" + "_" + System.DateTime.Now.Day.ToString() + System.DateTime.Now.Month.ToString() + System.DateTime.Now.Year.ToString() + System.DateTime.Now.Hour.ToString() + System.DateTime.Now.Minute.ToString() + System.DateTime.Now.Second.ToString() + ".pdf";
