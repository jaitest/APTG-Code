...
namespace BookAPOnlineHSRPRecords
{
    public partial class Form1 : Form
    {
        ...
        public void button1_Click(object sender, EventArgs e)
        {
            ...
            //AP_WithOut_PRNO//string sqlquery = "Select affixationcentercode,(select Rtolocationname from rtolocation where affixationcentercode=AffixationRTOCode)as 'rtoName',affixationCenterCode,H.transactionNo,H.transactionDate,H.authorizationRefNo,authorizationDate,engineNo,chassisNo,prNumber,ownerName,ownerAddress,ownerEmailId,ownerPinCode,mobileNo,vehicleType,transType,vehicleClassType,mfrsName,modelName,hsrpFee,oldNewFlag,govtVehicleTag,timeStamp,trNumber,dealerName,dealerMail,dealerRtoCode,regDate from HsrpData H,Transaction_Stagging T where H.transactionNo=T.transactionNo and H.transactionDate=T.transactionDate and H.hsrpFee=T.amount and H.authorizationRefNo=T.authorizationRefNo and affixationCenterCode=affixationcentercode and isnull(prNumber,'')='' and isnull(T.authorizationRefNo,'')!='' and isnull(T.transactionNo,'')!='' and Convert(date,T.recordCreationDate) between '2016-11-08' and '2016-11-22' and  H.authorizationRefNo not in (Select hsrprecord_Authorizationno from hsrprecords where hsrp_stateid=9)";
            //AP_with_PRUPDATE 
            //and Convert(date,T.transactionDate) = '2016-12-06'
            //string sqlquery = "Select h.rtocode as 'RTOCode',(select Rtolocationname from rtolocation where h.rtocode=AffixationRTOCode)as 'rtoName',affixationCenterCode,H.transactionNo,H.transactionDate,H.authorizationRefNo,authorizationDate,engineNo,chassisNo,prNumber,ownerName,ownerAddress,ownerEmailId,ownerPinCode,mobileNo,vehicleType,transType,vehicleClassType,mfrsName,modelName,hsrpFee,oldNewFlag,govtVehicleTag,timeStamp,trNumber,dealerName,dealerMail,dealerRtoCode,regDate from HsrpData H,Transaction_Stagging T where H.transactionNo=T.transactionNo and H.transactionDate=T.transactionDate and H.hsrpFee=T.amount and H.authorizationRefNo=T.authorizationRefNo and affixationCenterCode=affixationcentercode and isnull(prNumber,'')!='' and isnull(T.authorizationRefNo,'')!='' and isnull(T.transactionNo,'')!='' and T.authorizationRefNo in ('HSRPRTA1033801482418200','HSRPRTA1033911482418865') and  H.authorizationRefNo not in (Select hsrprecord_Authorizationno from hsrprecords where hsrp_stateid=9)";
