...
namespace BookAPOnlineHSRPRecords
{
    public partial class Form1 : Form
    {
        ...
        public void button1_Click(object sender, EventArgs e)
        {
            ...
            //AP_with_PRUPDATE //
            //string sqlquery = "Select affixationcentercode,(select Rtolocationname from rtolocation where affixationcentercode=AffixationRTOCode)as 'rtoName',affixationCenterCode,H.transactionNo,H.transactionDate,H.authorizationRefNo,authorizationDate,engineNo,chassisNo,prNumber,ownerName,ownerAddress,ownerEmailId,ownerPinCode,mobileNo,vehicleType,transType,vehicleClassType,mfrsName,modelName,hsrpFee,oldNewFlag,govtVehicleTag,timeStamp,trNumber,dealerName,dealerMail,dealerRtoCode,regDate from HsrpData H,Transaction_Stagging T where H.transactionNo=T.transactionNo and H.transactionDate=T.transactionDate and H.hsrpFee=T.amount and H.authorizationRefNo=T.authorizationRefNo and affixationCenterCode=affixationcentercode and isnull(prNumber,'')!='' and isnull(T.authorizationRefNo,'')!=''and isnull(T.transactionNo,'')!='' and isnull(T.transactionNo,'')!=''  and Convert(date,t.recordCreationDate)>= '2016-11-03' and  H.authorizationRefNo not in (Select hsrprecord_Authorizationno from hsrprecords where hsrp_stateid=9)";

            //string sqlquery = "Select h.rtocode as 'RTOCode',(select Rtolocationname from rtolocation where h.rtocode=AffixationRTOCode)as 'rtoName',affixationCenterCode,H.transactionNo,H.transactionDate,H.authorizationRefNo,authorizationDate,engineNo,chassisNo,prNumber,ownerName,ownerAddress,ownerEmailId,ownerPinCode,mobileNo,vehicleType,transType,vehicleClassType,mfrsName,modelName,hsrpFee,oldNewFlag,govtVehicleTag,timeStamp,trNumber,dealerName,dealerMail,dealerRtoCode,regDate from HsrpData H,Transaction_Stagging T where H.transactionNo=T.transactionNo and H.transactionDate=T.transactionDate and H.hsrpFee=T.amount and H.authorizationRefNo=T.authorizationRefNo and affixationCenterCode=affixationcentercode and isnull(prNumber,'')='' and isnull(T.authorizationRefNo,'')!='' and isnull(T.transactionNo,'')!='' and T.authorizationRefNo in ('HSRPRTA1626061485578946','HSRPRTA1630441485588184') and  H.authorizationRefNo not in (Select hsrprecord_Authorizationno from hsrprecords where hsrp_stateid=9)";
            
            string sqlquery = "Select affixationcentercode,(select Rtolocationname from rtolocation where affixationcentercode=AffixationRTOCode)as 'rtoName',affixationCenterCode,H.transactionNo,H.transactionDate,H.authorizationRefNo,authorizationDate,engineNo,chassisNo,prNumber,ownerName,ownerAddress,ownerEmailId,ownerPinCode,mobileNo,vehicleType,transType,vehicleClassType,mfrsName,modelName,hsrpFee,oldNewFlag,govtVehicleTag,timeStamp,trNumber,dealerName,dealerMail,dealerRtoCode,regDate from HsrpData H,Transaction_Stagging T where H.transactionNo=T.transactionNo and H.transactionDate=T.transactionDate and H.hsrpFee=T.amount and H.authorizationRefNo=T.authorizationRefNo and affixationCenterCode=affixationcentercode and isnull(prNumber,'')='' and isnull(T.authorizationRefNo,'')!='' and isnull(T.transactionNo,'')!='' and  H.authorizationRefNo not in (Select hsrprecord_Authorizationno from hsrprecords where hsrp_stateid=9)";
            //string sqlquery = "select affixationcentercode,(select Rtolocationname from rtolocation where affixationcentercode=AffixationRTOCode)as 'rtoName',affixationCenterCode,H.transactionNo,H.transactionDate,H.authorizationRefNo,authorizationDate,engineNo,chassisNo,h.prNumber,ownerName,ownerAddress,ownerEmailId,ownerPinCode,mobileNo,vehicleType,transType,vehicleClassType,mfrsName,modelName,hsrpFee,oldNewFlag,govtVehicleTag,timeStamp,trNumber,dealerName,dealerMail,dealerRtoCode,h.regDate from hsrpdata h , APNewSOP_SBIMIS sb where h.transactionNo=sb.HSRPNO and H.hsrpFee=sb.amount and affixationCenterCode=affixationcentercode and sb.ISPAYMENTRECIEVED='Y' and isnull(h.prNumber,'')=''and isnull(H.authorizationRefNo,'')!='' and isnull(H.transactionNo,'')!='' and isnull(sb.HSRPNO,'')!='' and authorizationRefNo='HSRPRTA2046851491305557' and H.authorizationRefNo not in (Select hsrprecord_Authorizationno from hsrprecords where hsrp_stateid=9)";
            ...
            if (dtRecords.Rows.Count > 0)
            {
                ...
                {
                    if (dtRecords.Rows[0]["affixationcentercode"].ToString() == "AP905")
                    {
                        string updatesqlquery = "update hsrpdata set  affixationCentercode='AP105' where affixationCentercode='" + dtRecords.Rows[k]["affixationcentercode"].ToString() + "' and authorizationRefNo='" + dtRecords.Rows[k]["authorizationRefNo"].ToString() + "' and transactionNo='" + dtRecords.Rows[k]["transactionNo"].ToString() + "' ";
                        //Utils.ExecNonQuery(updatesqlquery, strProvider);
                        DataTable dtRecordsaffix = Utils.GetDataTable(updatesqlquery, strProvider);
                        string straffixationname = dtRecordsaffix.Rows[0]["affixationcentercode"].ToString();
                        RTOLocationID = "select top 1 isnull(RTOLocationID,114) as rtoCode from RTOLocation where HSRP_StateID=9 and ActiveStatus='Y' and Affixationrtocode='" + straffixationname.ToString() + "'";
                    }
                    else
                    {
                   ...
                   // RTOLocationID = "select top 1 isnull(RTOLocationID,114) as rtoCode from RTOLocation where HSRP_StateID=9 and ActiveStatus='Y' and Affixationrtocode='" + dtRecords.Rows[k]["RTOCode"].ToString() + "'";
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand("InsertAPOnlineRcords_Without_PRNO"))
                        {
