...
namespace HSRP.AP
{
    public partial class APEmbossing : System.Web.UI.Page
    {
        ...
        private void ShowGrid()
        {
            ...
            if (dropdownDuplicateFIle.SelectedItem.Text != "--Select RTO Name--")
            {
                if (HSRPStateID == "9" || HSRPStateID == "11")
                {
                    ...
                    SQLString = "select hsrprecordid,vehicleregno,hsrp_rear_lasercode,hsrp_Front_lasercode,HSRPRecord_AuthorizationNo from hsrprecords where hsrp_stateid ='" + DropDownListStateName.SelectedValue + "' and PdfFileName='" + dropdownDuplicateFIle.SelectedItem.ToString() + "' and orderstatus ='New Order' order by VehicleClass,VehicleType,VehicleRegNo";
        ...
        protected void btnGO_Click(object sender, EventArgs e)
        {
            ...
            {
                SQLString = "select  Distinct pdffilename   from hsrprecords  where hsrp_StateID='" + DropDownListStateName.SelectedValue + "' and rtolocationID='" + dropDownListClient.SelectedValue + "' and  pdffilename is not null and convert(date,Hsrprecord_creationdate) between convert(date,'" + OrderDate.SelectedDate + "') and  convert(date,'" + HSRPAuthDate.SelectedDate + "') and orderstatus='New Order' group by  pdffilename, convert(varchar,pdfdownloaddate,111)";
                Utils.PopulateDropDownList(dropdownDuplicateFIle, SQLString.ToString(), CnnString, "--Select File Name--");
                GridView1.DataSource = null;
                GridView1.DataBind();
                btnSave.Visible = false;
            ...
            try
            {
                ...
                for (int i = 0; i < GridView1.Rows.Count; i++)
                {
                    ...
                    if (chk.Checked == true)
                    {
                        ...
                        //////string Squery = "Select Embossedpipdatetime from hsrprecords where hsrprecordid='"+id+"' ";
                        //////DataTable dt = Utils.GetDataTable(Squery, CnnString);
                        //////string OrderEmbossingDate = "";
                        //////if (dt.Rows.Count > 0)
                        //////{
                        //////    OrderEmbossingDate = dt.Rows[0]["Embossedpipdatetime"].ToString();
                        //////}
                        //////else
                        //////{
                        //////    OrderEmbossingDate = System.DateTime.Now.ToString();
                        //////}
                        //  labtext = GridView1.Rows[i].Cells[0].Text;
                        sb.Append("update hsrprecords set hsrp_Front_lasercode='" + RearLaserCode.Text + "' ,hsrp_rear_lasercode='" + FLasercode.Text + "',orderstatus='Embossing Done',[OrderEmbossingDate]=isnull(Embossedpipdatetime,getdate()), embossinguserid='" + strUserID + "' where HSRPRecordID='" + id.Text + "'" + Environment.NewLine);
                    }
                }
                ...
                if (DropDownListStateName.SelectedValue.ToString() == "9")
                {
                    ...
                    for (int i = 0; i < GridView1.Rows.Count; i++)
                    {
                        ...
                        if (chk.Checked == true)
                        {
                            ...
                            string Embossedpipdatetime = "Select Embossedpipdatetime from hsrprecords where HSRPRecordID='"+id.Text+"'";
                            string EmbPipvalue = Utils.getScalarValue(Embossedpipdatetime, CnnString);
                            if (EmbPipvalue == "")
                            {
