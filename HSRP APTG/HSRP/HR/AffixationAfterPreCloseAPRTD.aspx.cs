...
namespace HSRP.HR
{
    public partial class AffixationAfterPreCloseAPRTD : System.Web.UI.Page
    {
                ...
                if (DropDownListStateName.SelectedValue.ToString() == "9")
                {
                    ...
                    for (int i = 0; i < GridView1.Rows.Count; i++)
                    {
                        ...
                        if (chk.Checked == true)
                        {
                            ...
                            string Json = "{\"affixationCenterCode\":\"" + RTOLocationID + "\",\"affixationDate\":\"" + System.DateTime.Now.ToString("dd/MM/yyyy") + "\",\"authRefNo\":\"" + lblAuthNo + "\"}";
                            List<Response> responsecodeAp = ConfirmAffixation(Json, Utils.Login());
                            string status = responsecodeAp[0].status.ToString();
                            string message = responsecodeAp[0].message.ToString();
                            sb1.Append("update hsrprecords set APwebservicerespAffMsg='" + message + "',APwebservicerespAff='" + status + "',APwebservicerespAffdate=Getdate() where HSRPRecordID='" + id.Text + "'" + Environment.NewLine);
        ...
        public List<Response> ConfirmAffixation(string locationJSON, string token)
        {
            string URL = System.Configuration.ConfigurationManager.AppSettings["APRTDConfirmAffixationURL"].ToString();
